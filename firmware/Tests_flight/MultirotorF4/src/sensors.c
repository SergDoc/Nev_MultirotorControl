...
void sensorsAutodetect(void)
{
    ...
    // Autodetect gyro hardware. We have MPU3050 or MPU6000.
    if (mpu6000Detect(&acc, &gyro, mcfg.gyro_lpf, &mcfg.mpu6000_scale)) {
...
retry:
    switch (mcfg.acc_hardware) {
        ...
        case 2: // MPU6000
            if (haveMpu6k) {
                mpu6000Detect(&acc, &gyro, mcfg.gyro_lpf, &mcfg.mpu6000_scale); // yes, i'm rerunning it again.  re-fill acc struct
                accHardware = ACC_MPU6000;
                if (mcfg.acc_hardware == ACC_MPU6000)
                    break;
            }
            ; // fallthrough
        case 3: // MMA8452
    ...
    if (sensors(SENSOR_ACC))
        acc.init();
    // this is safe because either mpu6000 or mpu3050 or lg3d20 sets it, and in case of fail, we never get here.
    gyro.init();
